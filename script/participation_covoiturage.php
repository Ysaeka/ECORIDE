...
require_once 'notes_conducteur.php';
require_once __DIR__ . '/mail_reservation.php';
...
if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['covoiturage_id'])) {
    ...
    if (!$trajet || $trajet['nb_place'] <= 0) {
        header("Location: index.php?page=reservation&id=$covoiturage_id&erreur=plus_de_place");
        exit();
    }
    ...
    if ($credit < $prix) {
        header("Location: index.php?page=reservation&id=$covoiturage_id&erreur=credit_insuffisant");
        exit();
    }
    ...
    try {
        ...
        header("Location: index.php?page=mes_trajets&message=reservation_ok");
        exit();
