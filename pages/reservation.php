            ...
            <div class = "resaResult">
                <h2>Réservation du trajet : <?= ucfirst($trajet['lieu_depart']) ?> → <?= ucfirst($trajet['lieu_arrivee']) ?></h2>
                    <span class = "detailsTrajet">
                        <p><strong><i class="fa-solid fa-calendar-days fa-sm" style="color: #148b4b;"></i> Date :</strong> <?= htmlspecialchars($date_depart)?> | <?= $heure_depart ?> → <?= $heure_arrivee ?></p>
                        <p><strong>Places restantes :</strong> <?= $trajet['nb_place'] ?> <i class="fa-solid fa-user fa-sm" style="color: #148b4b;"></i></p>
                        <p><strong>Prix par personne :</strong> <?= number_format($trajet['prix_personne'], 2) ?> €</p>
                    </span>
                ...
                <a href="index.php?page=covoiturages" class="btnRetour"> Retour à la liste </a>

                <?php if($trajet['nb_place'] > 0): ?>
                    <form method="POST" action="index.php?page=participation_covoiturage" onsubmit="return confirmerReservation();">
